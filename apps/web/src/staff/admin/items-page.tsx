...
function CategoryRow({ category, onToggle, onSaved }: { category: RequestCategoryAdmin; onToggle: () => void; onSaved: () => Promise<void> }) {
  ...
  return <><tr><td className="px-4 py-2 font-medium text-foreground"><AutoText text={category.name} translations={category.name_i18n} /></td><td className="px-4 py-2 text-muted">{t(`department.${category.department}`)}</td><td className="px-4 py-2"><Badge className={category.active ? 'bg-ok-bg text-ok-ink' : undefined}>{category.active ? t('staff.items.statusActive') : t('staff.items.statusInactive')}</Badge></td><td className="px-4 py-2 text-right whitespace-nowrap"><button type="button" className="cursor-pointer text-xs text-muted hover:text-foreground" onClick={() => setOpen((v) => !v)}>{t('staff.items.translations')}</button><span className="mx-1.5 text-line-strong">·</span><button type="button" className="cursor-pointer text-xs text-muted hover:text-foreground" onClick={onToggle}>{category.active ? t('staff.items.deactivate') : t('staff.items.reactivate')}</button></td></tr>{open && <tr><td colSpan={4} className="bg-surface-2 px-4 py-3"><NameTranslationsForm baseName={category.name} initial={category.name_i18n} onSave={async (name_i18n) => { await updateRequestCategoryTranslations(category.id, name_i18n); await onSaved() }} /></td></tr>}</>
}

function ItemRow({ item, onToggle, onSaved }: { item: RequestTypeAdmin; onToggle: () => void; onSaved: () => Promise<void> }) {
  const { t } = useLocale(); const [open, setOpen] = useState(false)
  return <><tr><td className="px-4 py-2 font-medium text-foreground"><AutoText text={item.name} translations={item.name_i18n} /></td><td className="px-4 py-2 text-muted">{item.description ? <AutoText text={item.description} translations={item.description_i18n} /> : '—'}</td><td className="px-4 py-2 tabular-nums text-muted">{item.available_quantity ?? '—'}</td><td className="px-4 py-2"><Badge className={item.active ? 'bg-ok-bg text-ok-ink' : undefined}>{item.active ? t('staff.items.statusActive') : t('staff.items.statusInactive')}</Badge></td><td className="px-4 py-2 text-right whitespace-nowrap"><button type="button" className="cursor-pointer text-xs text-muted hover:text-foreground" onClick={() => setOpen((v) => !v)}>{t('staff.items.translations')}</button><span className="mx-1.5 text-line-strong">·</span><button type="button" className="cursor-pointer text-xs text-muted hover:text-foreground" onClick={onToggle}>{item.active ? t('staff.items.deactivate') : t('staff.items.reactivate')}</button></td></tr>{open && <tr><td colSpan={5} className="bg-surface-2 px-4 py-3"><div className="space-y-4"><NameTranslationsForm label={t('staff.items.name')} baseName={item.name} initial={item.name_i18n} onSave={async (name_i18n) => { await updateRequestTypeTranslations(item.id, { name_i18n, description_i18n: item.description_i18n }); await onSaved() }} />{item.description && <NameTranslationsForm label={t('staff.items.description')} baseName={item.description} initial={item.description_i18n} onSave={async (description_i18n) => { await updateRequestTypeTranslations(item.id, { name_i18n: item.name_i18n, description_i18n }); await onSaved() }} />}</div></td></tr>}</>
}
